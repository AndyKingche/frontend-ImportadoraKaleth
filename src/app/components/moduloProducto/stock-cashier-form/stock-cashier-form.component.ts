...
declare let $: any;
@Component({
  selector: 'app-stock-cashier-form',
  templateUrl: './stock-cashier-form.component.html',
  styleUrls: ['./stock-cashier-form.component.css']
})
export class StockCashierFormComponent implements OnInit {
  ...
  //metodo para insertar productos a la lista para posteriormente ser ingresada a la base de datos 
  async consultar() {
    //   ////  this.codigoProducto(no hay)
    if (this.idProductoencontrado <= 0) {

      this.notificacion.showError('No existe el producto en el inventario del almacen', '**ERROR');
    } else {
